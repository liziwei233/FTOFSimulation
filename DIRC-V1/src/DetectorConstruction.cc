...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  const int PhotonDetNu  = int(QuartzW2/PhotonDetW)-2;
  G4double  PhotonDetGap = QuartzW2/PhotonDetNu-PhotonDetW;
  ...
  // ----------------- DIRC --------------------
  G4Tubs*
    solidDIRCTubs1 = new G4Tubs("DIRCTubs1", SectorR1, SectorR2, SectorZ+SectorT/2., 0., 360*deg);
  G4Tubs*
    solidDIRCTubs2 = new G4Tubs("DIRCTubs2", SectorR1, SectorR2, SectorZ-SectorT/2., 0., 360*deg);
  G4SubtractionSolid*
  ...
  G4Trd*  
    solidSector = new G4Trd("Sector", 0.5*SectorW2, 0.5*SectorW1, 0.5*SectorT,0.5*SectorT,0.5*SectorH);
  G4LogicalVolume* 
  ...
  G4VPhysicalVolume *physiQuartz = new G4PVPlacement(0, G4ThreeVector(0,0,QuartzR/2.),
      logicQuartz, "Quartz", logicSector, false, 0, checkOverlaps);
  ...
  G4Box*  
    solidPhotonDet = new G4Box("PhotonDet", PhotonDetW/2, PhotonDetT/2, PhotonDetH/2);
  G4LogicalVolume* 
  ...
  for(int j=0; j<PhotonDetNu;j++) 
  {
    ...
    new G4PVPlacement(0, G4ThreeVector(PhotonDet_posX, PhotonDet_posY, PhotonDet_posZ),
        logicPhotonDet, "PhotonDet", logicSector, false, j, checkOverlaps);   
  }      
