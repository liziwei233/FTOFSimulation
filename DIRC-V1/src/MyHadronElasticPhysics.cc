...
//
G4_DECLARE_PHYSCONSTR_FACTORY(MyHadronElasticPhysics);
//
G4ThreadLocal G4bool MyHadronElasticPhysics::wasActivated = false;
G4ThreadLocal G4HadronElastic* MyHadronElasticPhysics::neutronModel = 0;
G4ThreadLocal G4HadronicProcess* MyHadronElasticPhysics::neutronProcess = 0;

MyHadronElasticPhysics::MyHadronElasticPhysics(G4int ver)
  : G4VPhysicsConstructor("hElasticWEL_CHIPS"), verbose(ver)
{
  if(verbose > 1) { 
    G4cout << "### MyHadronElasticPhysics: " << GetPhysicsName() 
	   << G4endl; 
  }
}
...
void MyHadronElasticPhysics::ConstructParticle()
{
  // G4cout << "MyHadronElasticPhysics::ConstructParticle" << G4endl;
...
void MyHadronElasticPhysics::ConstructProcess()
{
